...
function SummaryTab({ trips }) {
  ...
  function exportCsv(filter) {
    ...
    const lines = [header.join(",")];
    rows.forEach((t) => {
      // A split trip's real business type/client/job number live per-split,
      // not on the trip itself — represent them as a readable summary here
      // rather than exporting blank columns for every split trip.
      const isSplit = t.splits && t.splits.length > 0;
      const businessTypeOut = isSplit ? "split" : (t.businessType || "");
      const clientOut = isSplit
        ? t.splits.map((s) => `${s.businessType === "chargeable" ? (s.client || "Chargeable") : "Admin"}${s.jobNumber ? " #" + s.jobNumber : ""}: ${s.amount}km`).join("; ")
        : (t.client || "");
      const jobNumberOut = isSplit ? "" : (t.jobNumber || "");
      const line = [
        t.date, t.timeOut, t.fromLocation, t.mileageOut,
        t.timeIn, t.toLocation, t.mileageIn, t.mileageIn - t.mileageOut,
        t.category, businessTypeOut, `"${clientOut.replace(/"/g, '""')}"`,
        `"${(t.purpose || "").replace(/"/g, '""')}"`, jobNumberOut,
